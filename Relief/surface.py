...
from mpl_toolkits import mplot3d as mplt
...
#
# X,Y = np.meshgrid(x,y)
# Z=0.12861723162963065*X + 0.0014024845304814665*Y + 3.0964608113924048
#
# fig = plt.figure()
...
def chassis_coordinates_N(x, y, wheelbase, track_wdt, side_dst, front_dst, back_dst):
...
def chassis_points_hgt(x, y):
    coord_arr = chassis_coordinates_N(x, y)
...
def find_wheel_coords(xs, ys, zs):
    wh1x = xs - find_east_distance_btw_wheels(xs + e, ys + e, zs, track_wdt / 2)
    wh1y = ys + find_south_distance_btw_wheels(xs + e, ys + e, zs, wheelbase / 2)
    wh1z = IDW(file_arr, wh1x, wh1y, e, cell)
    wh2x = xs + find_east_distance_btw_wheels(xs + e, ys + e, zs, track_wdt / 2)
    wh2y = ys + find_south_distance_btw_wheels(xs + e, ys + e, zs, wheelbase / 2)
    wh2z = IDW(file_arr, wh2x, wh2y, e, cell)
    wh3x = xs - find_east_distance_btw_wheels(xs + e, ys + e, zs, track_wdt / 2)
    wh3y = ys - find_south_distance_btw_wheels(xs + e, ys + e, zs, wheelbase / 2)
    wh3z = IDW(file_arr, wh3x, wh3y, e, cell)
    wh4x = xs + find_east_distance_btw_wheels(xs + e, ys + e, zs, track_wdt / 2)
    wh4y = ys - find_south_distance_btw_wheels(xs + e, ys + e, zs, wheelbase / 2)
...
err_no = 0
for i in bar(range(len(file_arr))):
    try:
        result = 1
        xs = file_arr[i, 0]
        ys = file_arr[i, 1]
        ...
        ch_coord = chassis_coordinates_N(wh_coord[0, 0], wh_coord[0, 1], wheelbase, track_wdt, side_dst, front_dst, back_dst)
        x_spacing = np.linspace(ch_coord[0, 0], ch_coord[1, 0], 2)
        y_spacing = np.linspace(ch_coord[0, 1], ch_coord[2, 1], 5)
...
np.savetxt('results.txt', file_result, delimiter=',')
